...
public sealed class ProcessConfiguredFeaturesJob : IProcessJob
{
    public async ValueTask Run()
    {
        ...
        var flowersJsonFile = new FileInfo(Path.Combine(Helpers.OutputPath, "flowers.json"));

        await using var treesJsonFileStream = treesJsonFile.Create();
        await using var flowersJsonFileStream = flowersJsonFile.Create();
        ...
        foreach (var file in Directory.GetFiles(Helpers.ConfiguredFeaturesPath, "*.json"))
        {
            ...
            if (type != "minecraft:flower")
                continue;

            elements.Add($"minecraft:{name}", element.GetProperty("config"));
        }

        await JsonSerializer.SerializeAsync(flowersJsonFileStream, elements, Helpers.CodecJsonOptions);
    }
}
